...
**但重启并不是"新代码生效"的前提——实测**：`0.6.2` → `0.6.3` 之后**没有重启**，下一轮注入的路径
就已经是 `...\token-stats\0.6.3\runtime\token-stats.mjs`。所以重新解析本身能拿到新版本；
重启真正解决的是**间歇性失败**（部分调用仍解析到已删目录），不是版本。
**结论：按流程做（安装后完全重启）的人碰不到这个窗口**，推迟删除因此只是可选加固，不是必须。
...
**0.7.0 已按这个方案实施。** `install.mjs` 不再删除被替换的旧版本目录：先 `copyDir` 新版本、写
`SOURCE`，再把 `cache/local/token-stats/` 按 mtime 裁到**只保留最新两个**，注册表晚于这两步——多留的
那一个就是飞行中会话还指着的路径。本会话给了它一条比 0.2.0 更硬的证据：0.6.3→0.7.0 之后连着两次钩子
失败（Stop 18:38:21、UserPromptSubmit 18:38:58，都是 `Plugin directory does not exist: …\0.6.3`），
所以"下一轮重新解析就自愈"不成立，root 是**按会话钉死**的，代价也不是丢一轮而是重启前全失能。
测试 41 → **42 passed**（新增一例：先塞三个更老的版本目录，升级后只剩最新两个）。
