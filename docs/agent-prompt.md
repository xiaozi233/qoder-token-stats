...
> **⚠ 0.7.0 起这一段已经改了，先读这条再看下面。**「让模型每轮自己跑 CLI」在 Auto
> 权限模式下躲不过 classifier：先以"与用户问候无关"拒，再以"执行 workspace 之外的
> 脚本"拒，攒够 3 次就触发会话级熔断。现在的链路是 Stop 钩子在本轮结束后测量并归档，
> 再用 `{"decision":"deny","reason":"…统计行…"}` 把这一行回注给模型，模型照抄贴出；
> 下一个带 `stop_hook_active` 的 Stop 只负责结算这次唤醒。模型全程不调用任何工具，
> 三个拦口一个都碰不到。下面凡是写"把 CLI 命令作为最后一个动作运行"的地方，都按这个
> 新机制理解；`--current --key` 只在用户主动要数字时手工使用。
