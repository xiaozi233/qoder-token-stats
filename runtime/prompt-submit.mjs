...
const INSTRUCTION = [
  '',
  '【本轮统计指令】如果本次回答调用过任何工具，请在收尾时（其他工作全部完成、输出最终总结文字之前）运行下面这条命令：',
  `node "${cli}" --current`,
  '它会输出一行本轮真实的 token 速率统计。把该行原样放进 Markdown 引用块（新行行首加「> 」）贴在回复最末尾；不要改写数字，不要追加别的说明。',
  '注意：①若命令没有任何输出，或本次回答没有调用工具，就不要显示统计行，也不要为此额外调用工具；②绝不要凭记忆或估算自行编造这行数字。',
].join('\n');
